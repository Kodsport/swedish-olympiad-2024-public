...
int32_t main()
{
	...
	vvp3 buckets(n / 64 + 10);
	dread(int, q);
	rep(i, q)
	{
		dread2(int, a, b);
		a--; b--;
		buckets[b/64].emplace_back(a, b, i);
	}
	vi ans(q);
	int j = 0;
	for (int g = 0; g < n+128; g+=64)
	{
		vull masks(n);
		vvp2 whoasked(n);

		repe(bucket, buckets[j])
		{
			int a, b, i;
			tie(a, b, i) = bucket;
			whoasked[a].emplace_back(b, i);
		}

		rep(i, n)
		{
			int u = order[i];
			if (u >= g && u < g + 64) masks[u] |= 1ULL<<(u & 63);

			repe(who, whoasked[u])
			{
				if (masks[u]&(1ULL<<(who.first&63)))
				{
					ans[who.second] = 1;
				}
			}
