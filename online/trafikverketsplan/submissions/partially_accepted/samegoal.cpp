...
void dfs(int u, vvi& queries, vi& yes, vvi& edges)
{
    repe(q, queries[u]) yes[q] = 1;

    repe(e, edges[u]) dfs(e, queries, yes, edges);
}
...
int32_t main()
{
    ...
    rep(i, q)
    {
        ...
        a--; b--;
        assert(b == 0);
        queries[a].emplace_back(i);
    }

    dfs(0, queries, yes, edges);
