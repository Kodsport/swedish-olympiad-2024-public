...
// elaborate cheese. only recomputes maxima/minima in set if needed
// can be improved, but I won't bother.
// insert is O(1), erase is O(size of set)

int l, r;
bool ansone()
{
    while (coversall.size() && !active[coversall.back()]) coversall.pop_back();

    return coversall.size();
}

int cachel = -1;
int cacher = -1;

void updatecache(int& cache, vp2& li, int mul)
{
    if (li.empty())
    {
        cache = -1;
    ...
    {
        cache = 0;
        rep(i, li.size()) if (li[i].first * mul > li[cache].first * mul) cache = i;
    }
    else
    {
        if (li.back().first * mul > li[cache].first * mul) cache = li.size() - 1;
    }
}

bool anstwo()
{
    if (isleft.empty() || isright.empty()) return 0;
    int rightmost = isleft[cachel].first;
    int leftmost = isright[cacher].first;
...
int32_t main()
{
    ...
    rep(i, q)
    {
        ...
        if (c == '+')
        {
            ...
            if (lo <= l)
            {
                isleft.emplace_back(hi, id);
                updatecache(cachel, isleft, 1);
            }
            if (hi >= r)
            {
                isright.emplace_back(lo, id);
                updatecache(cacher, isright, -1);
            }
        ...
        else
        {
            ...
            if (lo <= l)
            {
                ...
                rep(i, isleft.size())
                {
                    if (isleft[i].second==id)
                    {
                        isleft.erase(isleft.begin() + i);
                        break;
                    }
                }
                updatecache(cachel, isleft, 1);
            ...
            if (hi >= r)
            {
                ...
                rep(i, isright.size())
                {
                    if (isright[i].second == id)
                    {
                        isright.erase(isright.begin() + i);
                        break;
                    }
                }
                updatecache(cacher, isright, 1);
