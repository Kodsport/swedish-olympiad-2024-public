...
// 2 big heuristics:
// cache result of anstwo, reuse if not invalid
// ignore intervals which are contained by surveilled interval

int l, r;
bool ansone()
{
    for (auto& [l1, r1, _] : intervals)
    {
...
bool anstwo()
{
    if (cached.first != -1 && active[cached.first] && active[cached.second])
    {
        return 1;
    }
    else cached = p2(-1, -1);
    for (auto& [l1, r1, i] : intervals)
    {
        for (auto& [l2, r2, j] : intervals)
        {
            p2 a = { l1,r1 };
            p2 b = { l2,r2 };
            if (a.first > b.first) swap(a, b);
            if (a.first <= l && b.second >= r && a.second >= b.first)
            {
                cached = { i,j };
                return 1;
            }
...
int32_t main()
{
    ...
    rep(i, q)
    {
        ...
        {
            dread(int, lo, hi);
            if (lo<=l||hi>=r) intervals.emplace_back(lo, hi, id);
        }
        else
        {
            rep(i, intervals.size())
            {
                if (get<2>(intervals[i]) == id)
                {
