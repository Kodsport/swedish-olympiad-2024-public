...
int32_t main()
{
    ...
    rep(i, m)
    {
        RollingHash hasher(dict[i]);
        groupmap[dict[i].size()].insert(hasher.gethash(0, dict[i].size() - 1));
    }
    ...
    rep(i, n)
    {
        repe(g, groups)
        {
            ...
            bool works = setcontains(g.second, h.gethash(i, i + g.first - 1));
