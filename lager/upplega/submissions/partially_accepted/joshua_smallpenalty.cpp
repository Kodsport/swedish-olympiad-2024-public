...
signed main()
{
	...
	rep(i, 50)
	{
		...
		dpval b = best(1, 0, lambda, profitleft, profitright, dp);
		if (b.second==k)
		{
			cout << int(round(b.first + lambda * k)) << "\n";
			return 0;
		}
		if (b.second > k) lo = lambda;
		else hi = lambda;
	}
	dpval b = best(1, 0, lo, profitleft, profitright, dp);
	cout << int(round(b.first + lo * k)) << "\n";
