...
bool possible(vp2 buildings, int t, int goal)
{
    ...
    rep(i, goal)
    {
        ...
        rep(j, n)
        {
            int profit = buildings[j].first * daysleft - buildings[j].second;
            if (profit > bestv && moneys >= buildings[j].second)
            {
                bestv = profit;
                best = j;
            }
