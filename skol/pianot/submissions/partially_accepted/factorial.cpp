...
int32_t main()
{
    ...
    repe(v, notes) allnotes.insert(v);
    map<char, int> mapping;
    map<int, char> revmap;
    repe(v, allnotes) if (!setcontains(mapping, v)) mapping[v] = mapping.size();
    repe(v, mapping) revmap[v.second] = v.first;
    ...
    rep(i, notes.size() - 1)
    {
        int a = mapping[notes[i]];
        int b = mapping[notes[i + 1]];
        cost[a][b] += 1;
        cost[b][a] += 1;
    }
    vi perm;
