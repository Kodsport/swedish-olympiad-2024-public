...
while rind < 2*n:
    ...
    rind += 1
    if (rind==1 and lind == 0 and reklam[lind]==0) or (rind == n*2-1 and lind<rind and reklam[rind]==t):
        ads += 1
...
while lind < 2*n: #flyttar lind ett steg
    ...
    lind += 1
    if lind % 2:
        if (lind == 1 and reklam[0]==0) or (lind == 2*n-1 and reklam[2*n-1] == t):
            ads -= 1
        ads -= reklam[lind]-reklam[lind-1]
