...
void dfs(int u, int p, vvi& treeadj, vi& color, vi& fixed, vi& nodes)
{
	if (u != p)
	{
		color[u] ^= 1;
	...
	fixed[u] = 1;
	rep(i, treeadj[u].size())
	{
		int e = treeadj[u][i];
		if (e == p) continue;
		while (!fixed[e])
		{
			dfs(e, u, treeadj, color, fixed, nodes);
			nodes.push_back(u);
			color[u] ^= 1;
		}
	}
...
int dfs(int u, int p, vi& subtree, vvi& treeadj)
{
	int ret = 1;
	repe(e, treeadj[u]) if (e != p)ret += dfs(e, u, subtree, treeadj);
...
signed main()
{
	...
	vi subtree(qcnt);
	dfs(start, start, subtree, treeadj);
	repe(e, treeadj)
	{
	...
	while (1)
	{
		dfs(currp, currp, treeadj, color, fixed, nodes);
		if (fixed[start]) break;
		bool works = 1;
		repe(e, edges[start]) works &= fixed[e];
		if (works) break;
	}
