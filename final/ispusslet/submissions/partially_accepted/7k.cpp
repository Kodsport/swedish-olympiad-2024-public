...
signed main()
{
	...
	if (color[start] == 0)
	{
		...
		if (retstart)
		{
			...
			vi path = { cycle[best] };
			rep(i, k) path.push_back(cycle[(i + best + 1) % cycle.size()]);
			dopath(path);
			path = { cycle[best] };
			auto reduce = [&](int x) {return x >= 0 ? x : x + sz(cycle); };
			rep(i, k) path.push_back(cycle[reduce(best - i - 1)]);
			dopath(path);
			rep(i, sz(cycle))
			{
				int u = cycle[(best + i + 1) % cycle.size()];
				nodes.push_back(u);
				color[u] ^= 1;
			}
			...
			vi c2(qcnt);
			repp(i, 1, sz(nodes)) c2[nodes[i]] ^= 1;
			assert(c2 == color);
			nodes.pop_back();
